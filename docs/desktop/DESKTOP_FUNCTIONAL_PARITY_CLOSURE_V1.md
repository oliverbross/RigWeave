# Desktop Functional Parity Closure v1

Branch: `feature/desktop-functional-parity-closure-v1`

Immutable UI/source baseline: `de32c8ac908c7979f39bfdfc41ca050378901e75`.

## Contract

This programme replaces the 17 `FOUNDATION_WIRED` rows with production desktop owners while preserving the accepted Deep Convergence v2 layouts, native menus, command registry, sidebar, Edit Layout policy, design tokens, accessibility/focus model, responsive breakpoints and deterministic gallery profiles.

Source completion requires an owner, domain behavior, storage/migration where applicable, protocol/provider client, safe lifecycle, QML action binding, loading/empty/error truth, configuration/recovery, health/privacy, tests and package inclusion. Physical radio/audio/CAT/PTT/TUNE/RF, rotator movement, authenticated-provider, signing and release acceptance remain separate evidence.

## Safety baseline

- Restore is disconnected, RX-only and disarmed.
- Radio profile selection never connects.
- Transmit and rotator movement acceptance default false after create, migration and import.
- External spots/news/evidence never create Digi or Chaser eligibility.
- N1MM defaults disabled, loopback, untrusted, unarmed and non-restoring.
- Global Stop cancels provider work and disarms every operating workflow; physical RX/stop is claimed only after readback.
- Credentials stay in Windows Credential Manager/macOS Keychain aliases and never enter databases, configuration exports, diagnostics or support bundles.

## Phase 0 result

The baseline already has one canonical QSO owner, Wavelog sync owner, cluster/spot owner, generic Hamlib/TCI radio owner, Hamlib rotator owner, Panadapter owner, provider/cache policy, five versioned domain stores, accepted QML layouts, deterministic galleries and exact-SHA multi-platform CI. The remaining gap is executable domain behavior: most closure areas are registries, demo rows, review strings or disabled controls.

See `DESKTOP_FUNCTIONAL_GAP_LEDGER_V1.md` for the 17-row source audit and `DESKTOP_OWNER_GRAPH_V1.md` for ownership boundaries.

## Closure result

The authoritative matrix is `DESKTOP_FUNCTIONAL_PARITY_MATRIX.md`: 31 audited rows, 31 source-complete, zero `FOUNDATION_WIRED` and zero `MISSING`. Source-complete rows with credentials, hardware, audio, RF, transmission or movement requirements retain `LIVE_ACCEPTANCE_PENDING` boundaries.

Implementation evidence includes native serial/TCP radio and rotator fixtures, linked Rust modem/audio sessions, Groups.io fake authentication/delivery, migration/reopen checks, application/QML contracts and the 100k-QSO/domain scale probe. `DESKTOP_FUNCTIONAL_VISUAL_REGRESSION_V1.md` records the protected UI result. Hardware and provider procedures are deliberately separate in `DESKTOP_HARDWARE_ACCEPTANCE.md` and `DESKTOP_PROVIDER_ACCEPTANCE.md`.

Hosted workflow/package results and hashes are attached to the exact candidate SHA. No row is promoted solely because its layout, fixture or hosted build exists.
