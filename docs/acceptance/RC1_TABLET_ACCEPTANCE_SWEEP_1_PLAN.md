...
| RW-TAS1-005 | Intelligence / RF Globe | P1 | Open RF Globe; `054-intelligence-rf-globe.png` / `.xml` | Globe has an understandable geographic reference and bounded overlays | Only a blue sphere was drawn; a hard-coded 18 dp diagonal line crossed the full canvas | Add offline graticule/coarse coastline orientation layer and remove unbounded line; source contract test | FIXED |
...
No issue is silently omitted: the two broader UX requests are explicitly retained as post-RC1 follow-ups rather than being folded into the frozen acceptance candidate.
