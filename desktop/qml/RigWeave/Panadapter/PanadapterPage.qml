...
Item { id: root
    ...
    ColumnLayout { anchors.fill: parent; anchors.margins: 18; spacing: 10
        ...
        RowLayout { Layout.fillWidth: true
            ...
            Label { text: "FIT " + Panadapter.fittedFloorDb.toFixed(1) + " / " + Panadapter.fittedTopDb.toFixed(1) + " dB"; color: "#e3c765" }
        }
        RowLayout { Layout.fillWidth: true; visible: !Panadapter.fitAutoContrast
            Label { text: "Manual floor"; color: "#98a0a6" }
            SpinBox { from: -140; to: -20; value: Panadapter.manualFloorDb; onValueModified: Panadapter.manualFloorDb = value }
            Label { text: "Manual top"; color: "#98a0a6" }
            SpinBox { from: -120; to: 20; value: Panadapter.manualTopDb; onValueModified: Panadapter.manualTopDb = value }
            Label { text: "Peak decay dB/s"; color: "#98a0a6" }
            Slider { Layout.fillWidth: true; from: 0; to: 20; value: Panadapter.peakDecay; onMoved: Panadapter.peakDecay = value }
        }
        RowLayout { Layout.fillWidth: true
        ...
        Rectangle { id: viewport; Layout.fillWidth: true; Layout.fillHeight: true; color: "#101316"; border.color: "#3a4147"; clip: true
            ...
            Repeater { model: Spots
                Rectangle { required property real frequencyHz; required property string callsign; property real normalized: Panadapter.normalizedForFrequency(frequencyHz, scene.zoom, scene.pan); visible: normalized >= 0 && normalized <= 1; x: normalized * viewport.width - 3; y: 22; width: 7; height: 7; radius: 4; color: root.selectedMarkerFrequency === frequencyHz ? "#f2efe7" : "#d38b22"
                    TapHandler { onTapped: root.selectedMarkerFrequency = frequencyHz }
                    ToolTip.visible: markerHover.hovered; ToolTip.text: callsign + " · select, then explicit QSY"
                    HoverHandler { id: markerHover }
                }
            }
            Rectangle { anchors.horizontalCenter: parent.horizontalCenter; y: parent.height * scene.spectrumRatio - 2; width: parent.width; height: 4; color: "#3a4147"; opacity: Panadapter.displayMode === "Spectrum + waterfall" ? 1 : 0
                DragHandler { id: splitDrag; yAxis.enabled: true; xAxis.enabled: false; onActiveChanged: if (!active) scene.spectrumRatio = Math.max(.2, Math.min(.8, centroid.position.y / viewport.height)) }
            }
        ...
        Rectangle { Layout.fillWidth: true; implicitHeight: 12; radius: 2
            gradient: Gradient { orientation: Gradient.Horizontal; GradientStop { position: 0; color: "#101316" } GradientStop { position: .5; color: Panadapter.colourMap === "Viridis" ? "#2c7d78" : "#7a4d24" } GradientStop { position: 1; color: Panadapter.colourMap === "Thermal mono" ? "white" : "#f0b24c" } }
            Label { anchors.right: parent.right; anchors.bottom: parent.top; text: "Colour scale · older rows downward"; color: "#98a0a6"; font.pixelSize: 10 }
        }
    }
}
