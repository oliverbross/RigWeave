...
WorkspaceCanvas {
    ...
    CanvasPanel {
        ...
        ColumnLayout { anchors.fill: parent
            RowLayout { Layout.fillWidth: true
                ...
                Label { text: Spots.count + " observations"; color: "#98a0a6" }
            }
    ...
    CanvasPanel {
        ...
        Rectangle { id: map; anchors.fill: parent; color: "#101316"; border.color: "#3a4147"; clip: true
            ...
            ListView { anchors.fill: parent; anchors.leftMargin: layoutMode.currentIndex===0?76:0; interactive: false; model: Spots
                delegate: Item { required property int index; required property double frequencyHz; required property string callsign; required property string mode; required property int ageSeconds; width: layoutMode.currentIndex===2?map.width/2-40:ListView.view.width; height: 1; visible: frequencyHz>=root.lowHz&&frequencyHz<=root.highHz; x:layoutMode.currentIndex===1?((frequencyHz-root.lowHz)/(root.highHz-root.lowHz))*(map.width-180):layoutMode.currentIndex===2?(index%2)*map.width/2+24:0; y:layoutMode.currentIndex===1?60+index*42:layoutMode.currentIndex===2?Math.floor(index/2)*map.height/2+64:(1-(frequencyHz-root.lowHz)/(root.highHz-root.lowHz))*(map.height-30)
                    Rectangle { width:10; height:2; color:"#d38b22" }
                    Rectangle { x:12; y:-10; width:Math.max(120,label.implicitWidth+12); height:22; color:"#22272b"; border.color:"#d38b22"; Label { id:label; anchors.centerIn:parent; text:callsign+"  "+mode+"  "+ageSeconds+"s"; color:"#f2efe7"; font.pixelSize:11; font.bold:true } }
