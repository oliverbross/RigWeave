...
Item { ConfirmDialog{id:confirm;title:"Confirm physical movement";message:"Move the explicitly connected rotator to AZ "+Rotator.preparedAzimuth.toFixed(1)+" / EL "+Rotator.preparedElevation.toFixed(1)+"? Preparation alone never moves.";onAccepted:Rotator.confirmMove()}
    ...
    ColumnLayout{anchors.fill:parent;anchors.margins:18;spacing:14
        SafetyBanner{Layout.fillWidth:true;text:"Default disconnected and automation disarmed. Manual, prompt and selected-target policies all require explicit target, arm, fresh position, safe path, band assignment and TX policy."}
        ...
        GroupBox{title:"Limits and assignment";Layout.fillWidth:true;GridLayout{anchors.fill:parent;columns:4;Label{text:"Az limits"}Label{text:"0°–450°";color:"#f2efe7"}Label{text:"El limits"}Label{text:"0°–180°";color:"#f2efe7"}Label{text:"Forbidden sectors"}Label{text:"None configured";color:"#e3c765"}Label{text:"Automation arm"}Label{text:"SESSION ONLY / OFF";color:"#e3c765"}}}
        EmptyState{Layout.fillWidth:true;Layout.fillHeight:true;title:"No physical movement evidence";detail:"Native protocol codecs and Hamlib dummy fixtures validate state/safety. Real Windows movement, forbidden-sector path proof and PTT/RF coexistence remain LIVE_ACCEPTANCE_PENDING."}
    }
}
