...
class DesktopPanadapter final : public QObject {
    ...
    Q_PROPERTY(bool fitAutoContrast READ fitAutoContrast WRITE setFitAutoContrast NOTIFY settingsChanged)
    ...
    Q_PROPERTY(QString colourMap READ colourMap WRITE setColourMap NOTIFY settingsChanged)
    Q_PROPERTY(QString displayMode READ displayMode WRITE setDisplayMode NOTIFY settingsChanged)
    Q_PROPERTY(int fftSize READ fftSize WRITE setFftSize NOTIFY settingsChanged)
    Q_PROPERTY(int waterfallRows READ waterfallRows WRITE setWaterfallRows NOTIFY settingsChanged)
    Q_PROPERTY(int averageFrames READ averageFrames WRITE setAverageFrames NOTIFY settingsChanged)
    Q_PROPERTY(bool peakHold READ peakHold WRITE setPeakHold NOTIFY settingsChanged)
    Q_PROPERTY(double peakDecay READ peakDecay WRITE setPeakDecay NOTIFY settingsChanged)
    Q_PROPERTY(double manualFloorDb READ manualFloorDb WRITE setManualFloorDb NOTIFY settingsChanged)
    Q_PROPERTY(double manualTopDb READ manualTopDb WRITE setManualTopDb NOTIFY settingsChanged)
public:
    ...
    bool restoreConfiguration(const QVariantMap &configuration, QString *error = nullptr);
    ...
    Q_INVOKABLE qulonglong frequencyAt(double normalizedX, double zoom, double pan) const;
    Q_INVOKABLE double normalizedForFrequency(qulonglong frequencyHz, double zoom, double pan) const;
...
private:
    class Sink;
    struct Context;
    std::shared_ptr<Context> context(const QString &receiverId, int sampleRate, bool swapIq);
