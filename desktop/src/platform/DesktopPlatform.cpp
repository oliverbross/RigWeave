    ...
    if (value.metaType().id() == QMetaType::QVariantMap) {
        ...
        for (auto it = input.cbegin(); it != input.cend(); ++it) {
            ...
            const bool unsafe = key == "connected" || key == "ptt" || key == "tune" ||
                                key == "armed" || key == "moving" || key == "pendingcommands" ||
                                key == "livespots" || key == "providerbodies" || key == "qsodata" ||
                                key == "token" || key == "password" || key == "secret" || key == "apikey";
...
QVariantMap DesktopConfigurationManager::previewImport(const QString&path)const{QFile file(path);if(!file.open(QIODevice::ReadOnly)||file.size()>1048576)return{{"valid",false},{"error","Cannot read bounded configuration bundle"}};QJsonParseError parse;const auto doc=QJsonDocument::fromJson(file.readAll(),&parse);if(parse.error!=QJsonParseError::NoError||!doc.isObject())return{{"valid",false},{"error","Invalid JSON"}};QVariantList sections;for(auto it=doc.object().begin();it!=doc.object().end();++it)if(safeSection(it.key()))sections<<QVariantMap{{"name",it.key()},{"changed",m_root.value(it.key())!=it.value().toVariant()}};return{{"valid",true},{"sections",sections},{"restoreSafety","Radio disconnected; PTT/TUNE unavailable; rotator disarmed; pending commands excluded."}};}
bool DesktopConfigurationManager::applyImport(const QString&path,const QStringList&sections,QString*error){const auto preview=previewImport(path);if(!preview.value("valid").toBool()){if(error)*error=preview.value("error").toString();return false;}QFile file(path);file.open(QIODevice::ReadOnly);const auto incoming=QJsonDocument::fromJson(file.readAll()).object().toVariantMap();const auto before=m_root;for(const auto&section:sections)if(safeSection(section)&&incoming.contains(section))m_root[section]=incoming.value(section);if(!save(error)){m_root=before;save();return false;}emit changed();return true;}
