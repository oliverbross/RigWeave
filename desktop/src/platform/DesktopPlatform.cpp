...
QVariantMap DesktopConfigurationManager::previewImport(const QString&path)const{QFile file(path);if(!file.open(QIODevice::ReadOnly)||file.size()>1048576)return{{"valid",false},{"error","Cannot read bounded configuration bundle"}};QJsonParseError parse;const auto doc=QJsonDocument::fromJson(file.readAll(),&parse);if(parse.error!=QJsonParseError::NoError||!doc.isObject())return{{"valid",false},{"error","Invalid JSON"}};const QJsonObject object=doc.object();QVariantList sections;QStringList unknownSections;for(auto it=object.cbegin();it!=object.cend();++it){if(it.key()=="version")continue;if(safeSection(it.key()))sections<<QVariantMap{{"name",it.key()},{"changed",m_root.value(it.key())!=it.value().toVariant()}};else unknownSections<<it.key();}return{{"valid",true},{"sections",sections},{"unknownSections",unknownSections},{"requiresReview",!unknownSections.isEmpty()},{"restoreSafety","Radio disconnected; PTT/TUNE unavailable; rotator disarmed; pending commands excluded."}};}
