...
public:
    explicit GalleryTciServer(QObject*parent=nullptr):QObject(parent),m_server(QStringLiteral("RigWeave deterministic gallery TCI"),QWebSocketServer::NonSecureMode,this){
        m_timer.setInterval(45);connect(&m_timer,&QTimer::timeout,this,[this]{sendIq();});
        connect(&m_server,&QWebSocketServer::newConnection,this,[this]{while(m_server.hasPendingConnections()){QWebSocket*peer=m_server.nextPendingConnection();peer->setParent(this);m_peers.push_back(peer);connect(peer,&QWebSocket::textMessageReceived,peer,[peer](const QString&message){if(!message.contains("trx:0,true")&&!message.contains("trx:1,true")&&!message.contains("tune:0,true")&&!message.contains("tune:1,true"))peer->sendTextMessage(message);});connect(peer,&QWebSocket::disconnected,peer,&QObject::deleteLater);QTimer::singleShot(10,peer,[peer]{peer->sendTextMessage("start;protocol:GalleryFixture,1.0;device:Deterministic fake TCI;trx_count:2;channels_count:2;dds:0,14074000;dds:1,7074000;vfo:0,0,14074000;vfo:1,0,7074000;modulation:0,digu;modulation:1,cw;rx_enable:0,true;rx_enable:1,true;ready;");});QTimer::singleShot(40,this,[this]{if(!m_timer.isActive())m_timer.start();});}});
    }
...
private:
    void sendIq(){m_peers.removeIf([](const QPointer<QWebSocket>&peer){return peer.isNull();});for(int receiver=0;receiver<2;++receiver){std::vector<float>values(8192);for(int sample=0;sample<4096;++sample){const double phase=2.0*3.141592653589793*(receiver?311.0:173.0)*sample/4096.0+double(m_sequence)*.025;values[2*sample]=float(std::cos(phase)*.28);values[2*sample+1]=float(std::sin(phase)*.28);}const auto bytes=rigweave::tci::build_binary_for_test(rigweave::tci::DataType::Iq,static_cast<std::uint32_t>(receiver),96'000U,2U,values);const QByteArray frame(reinterpret_cast<const char*>(bytes.data()),static_cast<qsizetype>(bytes.size()));for(const auto&peer:m_peers)if(peer&&peer->state()==QAbstractSocket::ConnectedState)peer->sendBinaryMessage(frame);}++m_sequence;}
...
void captureGallery(QGuiApplication &app, DesktopApplication &desktop, QQuickWindow *window,
    ...
    const QList<GalleryFrame> frames = {
        ...
        {"Radio", "Radio-generic"}, {"Radio","TCI-disconnected-profile",0},
        {"Radio","TCI-connected-fake-server",1},{"Radio","TCI-two-receivers",2},
        {"Radio","TCI-receiver-switch",3},{"Digi", "Digi"},
        {"Panadapter","Panadapter-spectrum-waterfall",0},{"Panadapter","Panadapter-FIT-auto-contrast",1},
        {"Panadapter","Panadapter-manual-floor-top",2},{"Panadapter","Panadapter-peak-passband",3},
        {"Panadapter","Panadapter-dual-receiver",4},
        {"EQ", "EQ"}, {"Logbook", "Logbook"},
        {"Intelligence","RF-Flat-Filters",0},{"Intelligence","RF-Flat-20m-Observed-Heat",1},
        {"Intelligence","RF-Globe-Default",2},{"Intelligence","RF-Globe-15m",3},
        {"Intelligence","RF-Globe-Selected-Path",4},{"Intelligence","RF-Empty-Stale-Offline",5},
    ...
    *step = [&app, &desktop, window, directory, frames, index, step] {
        ...
        const int radioBackend=frame.fileName.startsWith("TCI-")?9:(frame.fileName=="Radio-generic"?7:0);
        ...
        QTimer::singleShot(350, window, [window, directory, frame, index, step, &app, &desktop] {
            desktop.setGalleryVariant(frame.destination,frame.variant);
            if (QObject *backend = window->findChild<QObject *>("radioBackend"))
                backend->setProperty("currentIndex",frame.fileName.startsWith("TCI-")?9:(frame.fileName=="Radio-generic"?7:0));
            ...
            if(frame.destination=="Intelligence"){
                for(QObject*tabs:window->findChildren<QObject*>("intelligenceTabs"))tabs->setProperty("currentIndex",2);
                for(QObject*projection:window->findChildren<QObject*>("rfProjection"))projection->setProperty("currentIndex",frame.variant>=2&&frame.variant<=4?1:0);
                for(QObject*map:window->findChildren<QObject*>("rfMapScene")){map->setProperty("zoom",frame.variant==4?1.35:1.0);map->setProperty("longitude",frame.variant==4?55.0:0.0);map->setProperty("latitude",frame.variant==4?12.0:0.0);}
            }
            QTimer::singleShot(150, window, [window, directory, frame, index, step, &app] {
...
int main(int argc, char *argv[]) {
    ...
    if(gallery){galleryTci=std::make_unique<GalleryTciServer>(&app);if(!galleryTci->start()||!desktop.prepareGalleryTci(galleryTci->url())){qCritical("Cannot start isolated deterministic gallery TCI fixture");return 4;}}
