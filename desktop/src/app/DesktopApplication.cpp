...
DesktopApplication::DesktopApplication(QObject*parent):QObject(parent),m_rfObservations(this),m_cluster(&m_spots,this),m_parity(this),m_supportBundle(&m_paths,this){connect(&m_radio,&DesktopRadioController::iqFrame,&m_panadapter,[this](const QString&id,quint32 rate,const QVector<float>&values){const QVariantMap receiver=m_radio.receivers()->receiver(id);quint64 centre=receiver.value("centreFrequencyHz").toULongLong();if(centre==0)centre=receiver.value("effectiveReceiveHz").toULongLong();m_panadapter.pushFloatIq(id,rate,values,centre,false);if(!m_panadapter.receiverIds().contains(m_panadapter.currentReceiverId()))m_panadapter.setCurrentReceiverId(id);});}
...
bool DesktopApplication::prepareGalleryTci(const QUrl&endpoint){if(!m_demoMode||!endpoint.isValid())return false;return m_radio.saveTciProfile({{"id","gallery-tci"},{"displayName","Deterministic fake TCI"},{"endpoint",endpoint.toString()},{"preferredIqSampleRate",96000},{"preferredReceiver",0},{"autoConnect",false},{"rxAudioOutputRoute",QString{}}});}
void DesktopApplication::setGalleryVariant(const QString&workspace,int variant){if(!m_demoMode)return;if(m_galleryVariant!=variant){m_galleryVariant=variant;emit galleryVariantChanged();}if(workspace=="Band Maps")m_parity.setGalleryBandMapLayout(variant);else if(workspace=="Radio"){if(variant==0)m_radio.disconnectRadio();else{if(m_radio.backend()!="tci")m_radio.connectTciProfile("gallery-tci");if(m_radio.receiverCount()>=2){if(variant==2){m_radio.selectActiveReceiver("tci:0");m_radio.selectListeningReceiver("tci:1");}else if(variant>=3){m_radio.selectActiveReceiver("tci:1");m_radio.selectListeningReceiver("tci:0");}}}}else if(workspace=="Panadapter"){m_panadapter.setDisplayMode("Spectrum + waterfall");m_panadapter.setPeakHold(true);m_panadapter.setPaused(false);if(variant==2){m_panadapter.setFitAutoContrast(false);m_panadapter.setManualFloorDb(-110);m_panadapter.setManualTopDb(-20);}else m_panadapter.setFitAutoContrast(true);if(variant==4&&m_panadapter.receiverIds().contains("tci:0"))m_panadapter.setCurrentReceiverId("tci:0");}else if(workspace=="Intelligence"){m_rfObservations.resetFilters();if(variant==1)m_rfObservations.setFilter("band","20m");else if(variant==3)m_rfObservations.setFilter("band","15m");else if(variant==4){m_rfObservations.setFilter("band","20m");m_rfObservations.setSelectedId("demo-live-ja");}else if(variant==5){m_rfObservations.setSelectedId({});m_rfObservations.setFilter("source","Unavailable provider");}}}

QVariantMap DesktopApplication::health()const{QString projectionError;const bool projection=m_database&&m_database->verifyProjection(&projectionError);return{{"database",m_database?"Open / schema 16":"Unavailable"},{"databaseRevision",m_database?QVariant::fromValue<qulonglong>(m_database->revision()):0},{"projection",projection?"Verified":projectionError},{"domainStores",m_parity.databaseHealth()},{"wavelog",m_wavelog?m_wavelog->state():"Unavailable"},{"cluster",m_cluster.state()},{"radio",m_radio.health()},{"rotator",m_rotator.state()},{"panadapter",m_panadapter.health()},{"rfObservations",QVariantMap{{"count",m_rfObservations.rowCount()},{"storedCount",m_rfObservations.storedCount()},{"rendererRecordCap",4096},{"droppedObservations",QVariant::fromValue<qulonglong>(m_rfObservations.droppedObservations())},{"sourceFreshness",m_rfObservations.filterSummary()},{"renderer","Flat and globe public Qt scene graph ready"},{"selectedId",m_rfObservations.selectedId()}}},{"configuration",m_configuration?"Loaded":"Unavailable"},{"providers",QStringLiteral("%1 registered; disabled by default").arg(m_parity.providers()->rowCount())}};}
