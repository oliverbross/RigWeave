...
QVariantMap DesktopApplication::health()const{QString projectionError;const bool projection=m_database&&m_database->verifyProjection(&projectionError);return{{"database",m_database?"Open / schema 16":"Unavailable"},{"databaseRevision",m_database?QVariant::fromValue<qulonglong>(m_database->revision()):0},{"projection",projection?"Verified":projectionError},{"domainStores",m_parity.databaseHealth()},{"wavelog",m_wavelog?m_wavelog->state():"Unavailable"},{"cluster",m_cluster.state()},{"radio",m_radio.state()},{"rotator",m_rotator.state()},{"panadapter",m_panadapter.state()},{"configuration",m_configuration?"Loaded":"Unavailable"},{"providers",QStringLiteral("%1 registered; disabled by default").arg(m_parity.providers()->rowCount())}};}
...
void DesktopApplication::globalStop(){m_parity.globalStop();m_rotator.stop();m_panadapter.stop();qInfo("Global STOP requested: reviews and provider work cancelled; rotator stop sent when connected; receive audio stopped; PTT/TUNE are unavailable");}
