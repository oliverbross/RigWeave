...
void DesktopApplication::shutdown(){if(m_shuttingDown)return;m_shuttingDown=true;emit shuttingDownChanged();if(m_adif)m_adif->cancel();if(m_wavelog)m_wavelog->close();m_cluster.disconnectProfile();m_panadapter.stop();m_rotator.stop();m_rotator.disconnectRotator();m_radio.disconnectRadio();m_supportBundle.close();if(m_configuration)m_configuration->save();BoundedLogger::shutdown();}
