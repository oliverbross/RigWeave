...
Android implements the KX3-style console, a Flex-specific receive-control workspace, the broader Neural DX workspace, Portable Chase, and POTA Activate. Apple implements a native iPad-focused navigation and radio/log/DX/panadapter flow. Their destination sets need not be forced into artificial parity; durable behaviour and evidence must remain clear.
...
- Flex console: keep radio/account/connection status above a two-region wide-tablet workspace for radios/diagnostics and the operating cockpit. The cockpit uses a real VITA Canvas for spectrum/waterfall, live meters, explicit attach/client ownership, multi-slice controls, PC audio and a separately scrollable session-gated TX region. Compact layouts stack the same content. Empty/offline instruments say they are waiting; no spectrum, meter or radio is fabricated. Settings → Radio owns the family switch and Flex selection hides KX-only EQ/physical-IQ destinations.
