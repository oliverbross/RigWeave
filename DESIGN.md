...
- Flex console: keep radio/account/connection status above a two-region wide-tablet workspace for radios/diagnostics and GUI stations/existing slices/receive controls. Compact layouts stack the same content. Large frequency, mode, filter, RX antenna and observed-TX metadata remain prominent; there are no TX controls, logo assets, spectrum or fabricated radios. Settings → Radio owns the family switch and Flex selection hides KX-only EQ/panadapter destinations.
