...
@Composable private fun SatelliteStatusPanel(controller:SatelliteOperationsController,history:(AmsatStatusSummary)->Unit){var search by rememberSaveable{mutableStateOf("")};var favouritesOnly by rememberSaveable{mutableStateOf(false)};val favouriteNames=controller.elements.rows.filter{it.noradId in controller.favourites}.map{it.name.substringBefore(' ').uppercase(Locale.US)};val rows=controller.status.rows.filter{row->row.displayName.contains(search,true)&&(!favouritesOnly||favouriteNames.any{row.name.uppercase(Locale.US).contains(it)})};Column(Modifier.fillMaxSize(),verticalArrangement=Arrangement.spacedBy(7.dp)){SatelliteProviderStrip(controller.status.metadata);Row(horizontalArrangement=Arrangement.spacedBy(7.dp),verticalAlignment=Alignment.CenterVertically){OutlinedTextField(search,{search=it},label={Text("Search status")},singleLine=true,modifier=Modifier.weight(1f));FilterChip(favouritesOnly,{favouritesOnly=!favouritesOnly},{Text("FAVOURITES")})};LazyColumn(Modifier.weight(1f),verticalArrangement=Arrangement.spacedBy(6.dp)){items(rows.take(250),key={it.name}){row->ElevatedCard(colors=CardDefaults.elevatedCardColors(containerColor=SatPanel)){Column(Modifier.padding(10.dp)){Text(row.displayName,color=SatInk,fontWeight=FontWeight.Bold);Text(listOf("Heard","Crew Active","Telemetry Only","Not Heard").joinToString(" · "){"$it ${row.counts[it]?:0}"},color=SatMuted);Text("24-hour community timeline · latest ${row.latestReportEpoch?.let{satelliteTime(it,true)}?:"none"} · advisory only",color=SatMuted);row.timeline.take(5).forEach{report->Text("${satelliteTime(report.reportedEpoch,true)} · ${report.status} · ${report.callsign.ifBlank{"anonymous"}}${report.grid.takeIf(String::isNotBlank)?.let{" · $it"}.orEmpty()}",color=SatMuted)};val next=controller.passes.firstOrNull{it.satellite.name.substringBefore(' ').let{n->row.name.contains(n,true)}};if(next!=null)Text("Next local prediction ${satelliteTime(next.pass.aos,controller.utc)} · ${"%.0f°".format(next.pass.maximumElevationDeg)}",color=SatAmber);TextButton({history(row)}){Text("SATELLITE LOGBOOK")}}}}}}}

@Composable private fun SatelliteTimersPanel(controller:SatelliteOperationsController){Column(Modifier.fillMaxSize(),verticalArrangement=Arrangement.spacedBy(7.dp)){SatelliteProviderStrip(controller.timers.metadata);Text("Timers unavailable: the reviewed provider is a human-facing HTML page with no stable machine-readable contract. Local SGP4 pass prediction is unaffected.",color=SatMuted);LazyColumn(Modifier.weight(1f),verticalArrangement=Arrangement.spacedBy(6.dp)){items(controller.timers.rows,key={it.id}){row->ElevatedCard(colors=CardDefaults.elevatedCardColors(containerColor=SatPanel)){Column(Modifier.padding(10.dp)){Text(row.satellite.ifBlank{"Satellite timer"},color=SatInk,fontWeight=FontWeight.Bold);Text("${row.label} · ${satelliteTime(row.startEpoch,true)} → ${satelliteTime(row.endEpoch,true)}",color=SatMuted);Text(if(row.functional)"VALIDATED ACTIVE WINDOW" else "INACTIVE",color=if(row.functional)SatGood else SatBad)}}}}}}
